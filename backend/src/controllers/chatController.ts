...
export const chatWithModel = async (req: Request, res: Response) => {
  ...
  const systemPrompt = `${models[modelType].systemPrompt}. 
  MODEL_CATEGORY = ${models[modelType].category}
...
NOTE: Do not reference  ${JSON.stringify(models)} in your answer, the response has nothing to do with it.
